...
Search with `python -m claude_memory search "<query>"`, which prints one row per hit as `type, date, title, content`, truncated. Add `--type code-pref` when you want conventions specifically — a plain query rarely surfaces them. Titles are unique handles: `python -m claude_memory dig "<title>"` returns that node in full, with its window, staleness, and links.
