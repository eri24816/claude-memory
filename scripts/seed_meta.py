...
`python -m claude_memory search "<query>" [--type <type>] [--limit N]` prints one row per hit as `type, date, title, content`, with content truncated. Pass `--type` whenever you want one kind of node — a plain query will not reliably surface a terse `code-pref` against a corpus that talks about Eric constantly. Titles are unique handles: `python -m claude_memory dig "<title>"` returns that node whole, with its time window, staleness, and links.
